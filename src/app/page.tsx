      ...
      <section className="grid gap-6 md:grid-cols-2">
        <div className="space-y-4 rounded-xl bg-white p-5 shadow">
          ...
          <label className="block space-y-1">
            ...
            <input className="w-full rounded border p-2" value={profile.name} onChange={(e) => setProfile((p) => ({ ...p, name: e.target.value }))} />
          </label>

          <label className="block space-y-1">
            <span className="text-sm font-medium">Target Role</span>
            <input className="w-full rounded border p-2" value={profile.targetRole} onChange={(e) => setProfile((p) => ({ ...p, targetRole: e.target.value }))} />
          </label>

          <label className="block space-y-1">
            <span className="text-sm font-medium">Skill Level</span>
            <select className="w-full rounded border p-2" value={profile.skillLevel} onChange={(e) => setProfile((p) => ({ ...p, skillLevel: e.target.value as ProfileInput['skillLevel'] }))}>
          ...
          <label className="block space-y-1">
            <span className="text-sm font-medium">Weekly Hours (1-40)</span>
            <input
              type="number"
              min={1}
              max={40}
              ...
              onChange={(e) => setProfile((p) => ({ ...p, weeklyHours: Number(e.target.value) || 1 }))}
            />
          </label>

          <label className="block space-y-1">
            <span className="text-sm font-medium">Interests</span>
            <textarea className="w-full rounded border p-2" rows={2} value={profile.interests} onChange={(e) => setProfile((p) => ({ ...p, interests: e.target.value }))} />
          </label>

          <label className="block space-y-1">
            <span className="text-sm font-medium">Strengths</span>
            <textarea className="w-full rounded border p-2" rows={2} value={profile.strengths} onChange={(e) => setProfile((p) => ({ ...p, strengths: e.target.value }))} />
          </label>

          <label className="block space-y-1">
            <span className="text-sm font-medium">Weak Areas</span>
            <textarea className="w-full rounded border p-2" rows={2} value={profile.weakAreas} onChange={(e) => setProfile((p) => ({ ...p, weakAreas: e.target.value }))} />
          </label>

          <label className="block space-y-1">
            <span className="text-sm font-medium">Tools / Stack</span>
            <input className="w-full rounded border p-2" value={profile.tools} onChange={(e) => setProfile((p) => ({ ...p, tools: e.target.value }))} />
          </label>

          <div className="flex gap-3">
            <button onClick={generate} className="rounded bg-brand px-4 py-2 text-white" disabled={loading}>
              {loading ? 'Generating...' : 'Generate Plan'}
            </button>
            <button onClick={() => setProfile(demoProfiles[0] as ProfileInput)} className="rounded border px-4 py-2" disabled={loading}>
              Load Demo
            </button>
          </div>
